...
// Bänke, Mülleimer. Positionen entlang der Gehwege.

import * as THREE from 'three';
import * as Mat from '../graphics/materials/MatLib.js';

export class Props {
  constructor({ roadNet, rand }) {
    ...
    // --- Laternenpositionen entlang aller Straßen, beidseitig versetzt
    const lampPos = [];
    const benchPos = [];
    const binPos = [];
    ...
    for (let i = 0; i < xs.length; i++) {
      for (let j = 0; j < segNS[i].length; j++) {
        ...
        for (let z = z0, side = 0; z < z1; z += 27, side++) {
          ...
          lampPos.push([xs[i] + sx * (halfX[i] + 1.0), z, Math.PI / 2 * sx]);
          if (rand.chance(0.12)) binPos.push([xs[i] + sx * (halfX[i] + 1.9), z + 4, 0]);
        }
      }
    }
    for (let j = 0; j < zs.length; j++) {
      for (let i = 0; i < segEW[j].length; i++) {
        ...
        const x0 = xs[i] + roadNet.boundX(i) + 4;
        const x1 = xs[i + 1] - roadNet.boundX(i + 1) - 4;
    ...
    lampPos.forEach(([x, z, rot], k) => {
      m.makeTranslation(x, 3.2 + 0.13, z);
      poleInst.setMatrixAt(k, m);
      q.setFromAxisAngle(Y, rot);
      // Ausleger ragt zur Straße
      const ax = x - Math.sin(rot) * 0.8 * 0 + Math.cos(rot) * 0; // Rotation unten via Matrix
      m.compose(new THREE.Vector3(x, 6.25 + 0.13, z), q, new THREE.Vector3(1, 1, 1));
      armInst.setMatrixAt(k, m);
      // Kopf am Auslegerende: lokal (0.8, 0, 0) rotiert
      const hx = x + Math.cos(rot) * 0.8;
      const hz = z - Math.sin(rot) * 0.8;
      m.compose(new THREE.Vector3(hx, 6.3 + 0.13, hz), q, new THREE.Vector3(1, 1, 1));
      this.headInst.setMatrixAt(k, m);
      this.lampPositions.push(new THREE.Vector3(hx, 6.0, hz));
    });
    ...
    if (benchPos.length) {
      ...
      benchPos.forEach(([x, z, rot], k) => {
        q.setFromAxisAngle(Y, rot);
        m.compose(new THREE.Vector3(x, 0.55, z), q, new THREE.Vector3(1, 1, 1));
